...
const ServiceItem = ({ title, description, details }) => {
  return (
    <div className="py-24 border-b border-white last:border-0">
      <motion.h2
        initial={{ opacity: 0, x: -50 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
        className="text-5xl md:text-6xl font-medium uppercase text-[#00CFE7] tracking-wide mb-10"
      >
        {title}
      </motion.h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10  ">
        <motion.p
          ...
          className="text-lg md:text-xl text-white font-light leading-relaxed max-w-xl"
        >
          {description}
        </motion.p>
        ...
        <div>
          ...
          <motion.h3
            ...
            className="text-[#00CFE7] text-lg font-medium uppercase tracking-widest mb-8"
          >
            Service Detail
          </motion.h3>
          <motion.ul
          ...
          >
            {details.map((item, index) => (
              <motion.li
                ...
                className="text-lg md:text-xl text-white font-light cursor-pointer hover:text-[#00CFE7] transition-colors duration-300 flex items-center group"
              >
                <span className="w-0 overflow-hidden group-hover:w-6 transition-all duration-300 opacity-0 group-hover:opacity-100 text-[#00CFE7] mr-0 group-hover:mr-2">
                  →
                </span>
                {item}
              </motion.li>
